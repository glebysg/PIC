...
robots = ["yumi","baxter"]
algorithms = ['fabrik','posecones', 'posecones', 'posecones']
algorithm_dirs = [d+"_"+str(precision)+"/" for d in ['rigid', 'rigid', 'soft_to_rigid', 'rigid_to_soft']]
...
for algorithm_index in range(len(algorithms)):
    algorithm_dir = algorithm_dirs[algorithm_index]
    algorithm = algorithms[algorithm_index]
    full_result = None
    for task in incision_tasks:
        for robot in robots:
                name = out_path+algorithm_dir+task+"_"+ robot + "_" + algorithm + ".txt"
                if path.exists(name):
...
fig, ax = plt.subplots()
if show_std:
    barplot_1 = ax.bar(x_pos, inc_CTEs, width,  yerr=inc_error, alpha=0.65, ecolor='black', edgecolor='black', linewidth=2, capsize=10, color='darkorange')
    ...
    barplot_1 = ax.bar(x_pos, inc_CTEs, width,  alpha=0.65, ecolor='black', edgecolor='black', linewidth=2, capsize=10, color='darkorange')
...
ax.set_xticklabels(['FABRIK',r'$\mathregular{PIC^R}$',r'$\mathregular{PIC^{S\rightarrow R}}$', r'$\mathregular{PIC^{R\rightarrow S}}$'])
...
plt.tight_layout()
plt.savefig(out_path+str(precision)+'_occ_graph.png')
plt.show()
...
# Save the figure and show
plt.tight_layout()
plt.savefig(out_path+str(precision)+'_dist_graph.png')
plt.show()
...
# Save the figure and show
plt.tight_layout()
plt.savefig(out_path+str(precision)+'_pose_graph.png')
plt.show()
